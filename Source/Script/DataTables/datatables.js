		...
		var FixedColumns = /** @class */ (function () {
			function FixedColumns(settings, opts) {
				...
				if (
					...
					throw new Error(
						"FixedColumns requires DataTables 2 or newer",
					);
				}
			...
			FixedColumns.prototype._addStyles = function () {
				...
				var scroller = $$1(dt.table().node()).closest(
					"div.dt-scroll-body",
				)[0];
				var rtl = this.s.rtl;
			...
			FixedColumns.prototype._setKeyTableListener = function () {
				...
				this.s.dt.on("key-focus.dt.dtfc", function (e, dt, cell) {
					...
					if (_this.c.start > 0) {
						...
						var rightMost = $$1(
							_this.s.dt.column(_this.c.start - 1).header(),
						);
						var rightMostPos = rightMost.offset();
						var rightMostWidth = rightMost.outerWidth();
						// If the current highlighted cell is left of the rightmost cell on the screen
						if (
							$$1(cell.node()).hasClass(_this.classes.fixedLeft)
						) {
							// Fixed columns have the scrollbar at the start, always
							scroll.scrollLeft(0);
						} else if (
							cellPos.left <
							rightMostPos.left + rightMostWidth
						) {
							// Scroll it into view
					...
					if (_this.c.end > 0) {
						...
						if (
							$$1(cell.node()).hasClass(_this.classes.fixedRight)
						) {
							scroll.scrollLeft(
		...
		_resizeAuto: function () {
			...
			if (this.c.details.renderer._responsiveMovesNodes) {
				...
				dt.rows({ page: "current" }).every(function (rowIdx) {
					...
					dt.cells(rowIdx, visibleColumns).every(
						function (rowIdx2, colIdx) {
			...
			if (
				...
				if (iTopRow != this.s.dt._iDisplayStart) {
					...
					if (this.s.dt.oFeatures.bServerSide) {
						...
						$(this.s.dt.nTable).triggerHandler(
