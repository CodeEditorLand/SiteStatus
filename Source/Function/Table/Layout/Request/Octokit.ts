...
import type { TOKEN_GITHUB_COMMIT_STATUS_CODE_EDITOR_LAND_WEBSITE } from "astro:env/server";
		...
		return (
			await new (await import("@octokit/rest")).Octokit({
				auth: (await import("astro:env/server")).getSecret(
					"TOKEN_GITHUB_COMMIT_STATUS_CODE_EDITOR_LAND_WEBSITE",
				) as typeof TOKEN_GITHUB_COMMIT_STATUS_CODE_EDITOR_LAND_WEBSITE,
