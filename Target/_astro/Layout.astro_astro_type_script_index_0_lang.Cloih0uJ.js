import { createNoise2D } from './simplex-noise.n4dlx2W3.js';
...
const HEX = (...[RGB]) => `#${RGB.map((_String) => {
  const HEX = _String.toString(16);
  return HEX.length === 1 ? `0${HEX}` : HEX;
}).join("")}`;

const Interpolate = (...[Previous, Next, By]) => [
...
const RGB = (...[HEX]) => {
  HEX = HEX.replace(/^#/, "");
  if (HEX.length === 3) {
    HEX = HEX.split("").map((_String) => _String + _String).join("");
  }
...
const TimeNoise$2 = 0;
const NoiseInfluence$2 = 0.5;
const Cell = (Element, Setting) => {
  const Cell = Element.querySelectorAll("td, th");
  const Total = Cell.length;
  if (!Total) {
    return false;
  }
  const Group = Math.ceil(Total / 3);
  const URL = Setting?.["ajax"]?.url;
  Cell.forEach((Element2, Index) => {
    let Low;
    let High;
    let Local;
    let Count;
    ...
    Element2.style.borderTopColor = HEX(
      Interpolate(
        Low,
        High,
        Lerp(
          Base,
          (Layer(TimeNoise$2 + Seed + 0, Local) + 1) / 2,
          NoiseInfluence$2
        )
      )
    );
    Element2.style.borderRightColor = HEX(
      Interpolate(
        Low,
        High,
        Lerp(
          Base,
          (Layer(TimeNoise$2 + Seed + 10, Local) + 1) / 2,
          NoiseInfluence$2
        )
      )
    );
    Element2.style.borderBottomColor = HEX(
      Interpolate(
        Low,
        High,
        Lerp(
          Base,
          (Layer(TimeNoise$2 + Seed + 20, Local) + 1) / 2,
          NoiseInfluence$2
        )
      )
    );
    Element2.style.borderLeftColor = HEX(
      Interpolate(
        Low,
        High,
        Lerp(
          Base,
          (Layer(TimeNoise$2 + Seed + 30, Local) + 1) / 2,
          NoiseInfluence$2
        )
      )
    );
...
const TimeNoise$1 = 0;
const NoiseInfluence$1 = 0.5;
const Input = (Element, _Setting) => {
  const Input = Element.querySelectorAll(".dt-input, .dt-button");
  const Total = Input.length;
  if (!Total) {
    return false;
  }
  const Group = Math.ceil(Total / 3);
  Input.forEach((Element2, Index) => {
    const Tag = Element2.classList.contains("Tag");
    let Low;
    let High;
    let Local;
    let Count;
    ...
    Element2.style.borderTopColor = HEX(
      Interpolate(
        ...
        Lerp(
          Base,
          (Layer(TimeNoise$1 + Seed + 0, Local) + 1) / 2,
          NoiseInfluence$1
        )
      )
    );
    Element2.style.borderRightColor = HEX(
      Interpolate(
        ...
        Lerp(
          Base,
          (Layer(TimeNoise$1 + Seed + 10, Local) + 1) / 2,
          NoiseInfluence$1
        )
      )
    );
    Element2.style.borderBottomColor = HEX(
      Interpolate(
        ...
        Lerp(
          Base,
          (Layer(TimeNoise$1 + Seed + 20, Local) + 1) / 2,
          NoiseInfluence$1
        )
      )
    );
    Element2.style.borderLeftColor = HEX(
      Interpolate(
        ...
        Lerp(
          Base,
          (Layer(TimeNoise$1 + Seed + 30, Local) + 1) / 2,
          NoiseInfluence$1
        )
      )
    );
    if (Element2 instanceof HTMLSelectElement) {
      Element2.style.backgroundImage = `url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3E%3Cpath stroke='rgb(${Interpolate(
        Low,
        High,
        Lerp(
          Base,
          (Layer(TimeNoise$1 + Seed + 40, Local) + 1) / 2,
          NoiseInfluence$1
        )
    ...
    Element2.style.backgroundColor = `rgb(${Interpolate(
      ...
      Lerp(
        Base,
        (Layer(TimeNoise$1 + Seed + 60, Local) + 1) / 2,
        NoiseInfluence$1
      )
    )})`;
  });
...
const Segment = (Total, Element) => {
  Element.innerHTML = "";
  Element.querySelectorAll(".Segment").forEach(
 ...
 *   https://datatables.net/download/#dt/jq-3.7.0/dt-2.2.2/b-3.2.2/fc-5.0.4/fh-4.0.1/r-3.0.4/rg-1.5.1/sc-2.4.3
 *
 * Included libraries:
 *   jQuery 3.7.0, DataTables 2.2.2, Buttons 3.2.2, FixedColumns 5.0.4, FixedHeader 4.0.1, Responsive 3.0.4, RowGroup 1.5.1, Scroller 2.4.3
 */
...
!(function (e, t) {
	...
	var oe = [],
		...
		C = ie.document,
		u = { type: true, src: true, nonce: true, noModule: true };
	function m(e, t, n) {
		var r,
			i,
	...
	!(function () {
		...
		{ radio: true, checkbox: true, file: true, password: true, image: true }))
			b.pseudos[e] = B(e);
		for (e in { submit: true, reset: true }) b.pseudos[e] = _(e);
		function G() {}
		function Y(e, t) {
			var n,
				r,
				...
				if ("string" != typeof e)
					return this.pushStack(
						ce(e).filter(function () {
							for (t = 0; t < r; t++)
								if (ce.contains(i[t], this)) return true;
						}),
					);
	...
	var E = /^(?:parents|prev(?:Until|All))/,
		j = { children: true, contents: true, next: true, prev: true };
	function A(e, t) {
	...
	ce.fn.extend({
		has: function (e) {
			...
			return this.filter(function () {
				for (var e = 0; e < n; e++)
					if (ce.contains(this, t[e])) return true;
			});
		},
	...
	(ce.fn.ready = function (e) {
		...
		ce.extend({
			...
			readyWait: 1,
			ready: function (e) {
				(true === e ? --ce.readyWait : ce.isReady) ||
					((ce.isReady = true) !== e && 0 < --ce.readyWait) ||
					O.resolveWith(C, [ce]);
			},
		}),
	...
	var R = function (e, t, n, r, i, o, a) {
			var s = 0,
				...
				l = null == n;
			if ("object" === x(n))
				for (s in ((i = true), n)) R(e, t, s, n[s], true, o, a);
			else if (
				void 0 !== r &&
				((i = true),
				v(r) || (a = true),
	...
	(xe = C.createDocumentFragment().appendChild(C.createElement("div"))),
		...
		xe.appendChild(be),
		(le.checkClone = xe.cloneNode(true).cloneNode(true).lastChild.checked),
		(xe.innerHTML = "<textarea>x</textarea>"),
		(le.noCloneChecked = !!xe.cloneNode(true).lastChild.defaultValue),
	...
	(ce.event = {
					...
					(d = g = (s = De.exec(e[l]) || [])[1]),
						...
						d &&
							...
							(p = u[d]) ||
								...
								(f.setup && false !== f.setup.call(t, r, h, a)) ||
									(t.addEventListener &&
				...
				while (l--)
					if (
						...
						a &&
							!p.length &&
							((f.teardown &&
								false !== f.teardown.call(e, h, v.handle)) ||
								ce.removeEvent(e, d, v.handle),
							delete u[d]);
					} else for (d in u) ce.event.remove(e, d + t[l], n, r, true);
		...
		dispatch: function (e) {
			...
			if (
				...
				!c.preDispatch || false !== c.preDispatch.call(this, u))
			) {
		...
		ce.each({ focus: "focusin", blur: "focusout" }, function (r, i) {
			...
			(ce.event.special[r] = {
				setup: function () {
					var e;
					if ((He(this, r, true), !C.documentMode)) return false;
	...
	var Oe = /<script crossorigin=\"anonymous\"|<style|<link/i,
	...
	function $e(n, r, i, o) {
		...
		if (
			f &&
			((t = (e = Ae(r, n[0].ownerDocument, false, n, o)).firstChild),
		...
		ce.fn.extend({
			...
			empty: function () {
				for (var e, t = 0; null != (e = this[t]); t++)
					1 === e.nodeType &&
						(ce.cleanData(Se(e, false)), (e.textContent = ""));
				return this;
			},
			clone: function (e, t) {
				return (
	...
	function it(e, t, n, r, i, o) {
		...
		for (; a < 4; a += 2)
			"margin" === n && (l += ce.css(e, n + Q[a], true, i)),
				r
					? ("content" === n &&
							(u -= ce.css(e, "padding" + Q[a], true, i)),
						"margin" !== n &&
							(u -= ce.css(e, "border" + Q[a] + "Width", true, i)))
					: ((u += ce.css(e, "padding" + Q[a], true, i)),
						"padding" !== n
							? (u += ce.css(e, "border" + Q[a] + "Width", true, i))
							: (s += ce.css(
	...
	function ot(e, t, n) {
		var r = Ue(e),
			i =
				...
				"border-box" === ce.css(e, "boxSizing", false, r),
			o = i,
			a = Ge(e, t, r),
		...
		return (
			((!le.boxSizingReliable() && i) ||
				...
				"auto" === a ||
				(!parseFloat(a) && "inline" === ce.css(e, "display", false, r))) &&
				e.getClientRects().length &&
				((i = "border-box" === ce.css(e, "boxSizing", false, r)),
				(o = s in e) && (a = e[s])),
			(a = parseFloat(a) || 0) +
			...
			ce.cssHooks[u] = {
				...
				set: function (e, t, n) {
					var r,
						...
						a =
							(o || n) &&
							"border-box" === ce.css(e, "boxSizing", false, i),
						s = n ? it(e, u, n, a, i) : 0;
					return (
						a &&
							o &&
	...
	function dt() {
		ut &&
			(false === C.hidden && ie.requestAnimationFrame
				...
				return (
					...
					this.each(function () {
						...
						for (t = n.length; t--; )
							n[t].elem !== this ||
								(null != i && n[t].queue !== i) ||
								(n[t].anim.stop(o), (e = false), n.splice(t, 1));
				...
				return (
					...
					this.each(function () {
						...
						for (
							t.finish = true,
								ce.queue(this, a, []),
								r && r.stop && r.stop.call(this, true),
								e = i.length;
							e--;
							...
							i[e].elem === this &&
								i[e].queue === a &&
								(i[e].anim.stop(true), i.splice(e, 1));
						for (e = 0; e < o; e++)
	...
	ce.extend(ce.event, {
		trigger: function (e, t, n, r) {
			...
			if (
				...
				3 !== n.nodeType &&
					...
					r || !c.trigger || false !== c.trigger.apply(n, t)))
			) {
				if (!r && !c.noBubble && !y(n)) {
					for (
						s = c.delegateType || d,
				...
				return (
					...
					r ||
						...
						(c._default && false !== c._default.apply(p.pop(), t)) ||
						!$(n) ||
						(u &&
							v(n[d]) &&
							!y(n) &&
		...
		simulate: function (e, t, n) {
			var r = ce.extend(new ce.Event(), n, { type: e, isSimulated: true });
		...
		ce.fn.extend({
			...
			triggerHandler: function (e, t) {
				var n = this[0];
				if (n) return ce.event.trigger(e, t, n, true);
			},
		});
	var qt = /\[\]$/,
		Lt = /\r?\n/g,
				...
				ce.each(t[e] || [], function (e, t) {
					...
					return "string" != typeof n || u || s[n]
						? u
							? !(r = n)
							: void 0
						: (i.dataTypes.unshift(n), l(n), false);
				}),
				r
			);
		}
	...
	(Ut.href = Et.href),
		ce.extend({
			...
			ajax: function (e, t) {
				...
				((v.data && v.hasContent && false !== v.contentType) ||
					t.contentType) &&
		...
		ce.fn.extend({
			wrapAll: function (e) {
				...
				return (
					this[0] &&
						(v(e) && (e = e.call(this[0])),
						(t = ce(e, this[0].ownerDocument).eq(0).clone(true)),
	...
	(le.cors = !!Qt && "withCredentials" in Qt),
		...
		ce.ajaxTransport("script", function (n) {
			...
			if (n.crossDomain || n.scriptAttrs)
				return {
					send: function (e, t) {
						(r = ce("<script crossorigin=\"anonymous\">")
							.attr(n.scriptAttrs || {})
				...
				if (this[0]) {
					...
					if ("fixed" === ce.css(r, "position"))
						...
						e &&
							...
							(i.left += ce.css(e, "borderLeftWidth", true)));
					}
					return {
						top: t.top - i.top - ce.css(r, "marginTop", true),
						left: t.left - i.left - ce.css(r, "marginLeft", true),
					};
				}
			},
...
!(function (n) {
	...
	var T,
						...
						this.each(function () {
							...
							if ("table" != this.nodeName.toLowerCase())
								...
								H(this).trigger("options.dt", e),
									...
									K(r.column),
									B(r, r, true),
									B(r.column, r.column, true),
									B(r, H.extend(e, a.data()), true);
								var o = V.settings;
								for (t = 0, j = o.length; t < j; t++) {
									...
									if (
										i.nTable == this ||
										(i.nTHead &&
											i.nTHead.parentNode == this) ||
										(i.nTFoot &&
											i.nTFoot.parentNode == this)
									) {
								...
								(null !== n && "" !== n) ||
									((n =
										"DataTables_Table_" + V.ext._unique++),
									(this.id = n));
								var u,
									c = H.extend(true, {}, V.models.oSettings, {
										sDestroyWidth: a[0].style.width,
										sInstance: n,
										sTableId: n,
										colgroup:
											H("<colgroup>").prependTo(this),
								...
								var N,
									...
									F =
										...
										c.oFeatures);
								if (
									(e.bStateSave && (F.bStateSave = true),
									void 0 === e.aaSorting)
								)
									for (
										var O = c.aaSorting,
								...
								var n = a.children("caption"),
									...
									R =
										...
										(c.nTFoot = n[0]),
										(c.aiDisplay =
											c.aiDisplayMaster.slice()),
										(c.bInitialised = true),
										c.oLanguage);
								H.extend(true, R, e.oLanguage),
									R.sUrl
										? H.ajax({
												dataType: "json",
												url: R.sUrl,
			...
			((V.ext = T =
				{
					...
					classes: {},
					builder:
						"dt/jq-3.7.0/dt-2.2.2/b-3.2.2/fc-5.0.4/fh-4.0.1/r-3.0.4/rg-1.5.1/sc-2.4.3",
					errMode: "alert",
					feature: [],
					features: {},
					search: [],
		...
		w = function (e) {
			...
			for (
				e = e.replace(F, "");
				(e = (t = e).replace(/<script crossorigin=\"anonymous\"/i, "")) !== t;
	...
	function te(e, t, n) {
		...
		(a._bAttrSrc =
			...
			"number" == typeof o || a._isArrayHost || (e._rowReadObject = true),
			e.oFeatures.bSort || (a.bSortable = false);
	}
	function ne(e) {
		!(function (t) {
			if (t.oFeatures.bAutoWidth) {
				...
				(t.containerWidth = f),
					...
					d && -1 !== d.indexOf("%") && (u = d),
					G(t, null, "column-calc", { visible: s }, false);
				var f = H(r.cloneNode())
	...
	function C(e, t) {
		if (
			...
			-1 !== G(e, "aoPreDrawCallback", "preDraw", [e]).indexOf(false))
		)
			L(e, false);
		else {
			var l,
				...
				u = e.aoColumns,
				c = H(e.nTBody);
			if (((e.bDrawing = true), e.deferLoading))
				(e.deferLoading = false), e.iDraw++, L(e, false);
			else if (a) {
				if (!e.bDestroying && !t)
					return (
			...
			G(e, "aoHeaderCallback", "header", [
				...
				G(e, "aoDrawCallback", "draw", [e], true),
				(e.bSorted = false),
				(e.bFiltered = false),
				(e.bDrawing = false);
		}
	...
	function Ie(t) {
		...
		if (((t.nTableWrapper = a[0]), t.sDom))
			...
			n.forEach(function (e) {
				...
				g(t, a, { full: { table: true, contents: [Be(t)] } }),
				e.forEach(function (e) {
					g(t, a, e);
				});
		}
	...
	function Ne(n, e, r) {
		function t(e) {
			...
			(n.json = e), G(n, null, "xhr", [n, e, n.jqXHR], true), r(e);
		}
		var a,
			o = n.ajax,
			i = n.oInstance,
			l =
				(H.isPlainObject(o) &&
					...
					(e = "function" == typeof a && l ? l : H.extend(true, e, l)),
					delete o.data),
				{
					...
					error: function (e, t) {
						-1 ===
							G(n, null, "xhr", [n, null, n.jqXHR], true).indexOf(
								true,
							) &&
							("parsererror" == t
	...
	function je(n, e) {
				...
				if (c[d] && !(l = c[d])._aFilterData) {
					for (o = [], r = 0, a = u.length; r < a; r++)
						u[r].bSearchable
							...
							o.push(i);
					(l._aFilterData = o), (l._sFilterRow = o.join("  "));
				}
		...
		(n.bFiltered = true), G(n, null, "search", [n]);
	}
		...
		i.bInitialised
			? (we(i, "header"),
				...
				(n = function () {
					...
					true === a ? (t = r[0]) : false === a && (t = r[r.length - 1]),
						$e(
							e,
							t,
							t === e.nTHead
	...
	function He(e) {
		...
		e._bInitComplete ||
			...
			ne(e),
			G(e, null, "plugin-init", t, true),
			G(e, "aoInitComplete", "init", t, true));
	}
	function We(e, t) {
		t = parseInt(t, 10);
	...
	function L(e, t) {
		(e.bDrawing && false === t) || G(e, null, "processing", [e, t]);
	}
	function Ve(e, t, n) {
		t
			? (L(e, true),
	...
	function Be(e) {
		...
		return "" === g.sX && "" === g.sY
			...
			: ((t = g.sX),
				...
				(u = H(p[0].cloneNode(false))),
				(i = H(p[0].cloneNode(false))),
				(c = function (e) {
					return e ? I(e) : null;
				}),
								...
								(r || n) && e.oFeatures.bSortMulti
									...
									: l.length && l[0][0] == t
										...
										: ((l.length = 0),
											l.push([t, s[0]]),
											(l[0]._idx = 0));
							})(o, n[r], r, e.shiftKey) && (t = true),
						1 === o.aaSorting.length && "" === o.aaSorting[0][1])
					)
						break;
				t &&
					Ve(o, true, function () {
						Ze(o), Ye(o, o.aiDisplay), d(o, false, false), l && l();
					});
			}
		});
	}
	...
	function et(r, e, t) {
		var n,
			...
			l =
				((r._bLoadingState = true),
				r._bInitComplete ? new V.Api(r) : null);
		if (e && e.time) {
			var s = r.iStateDuration;
			if (0 < s && e.time < +new Date() - 1e3 * s) r._bLoadingState = false;
			else if (
				-1 !==
				G(r, "aoStateLoadParams", "stateLoadParams", [r, e]).indexOf(false)
			)
				r._bLoadingState = false;
			else {
				if (
					((r.oLoadedState = H.extend(true, {}, e)),
					G(r, null, "stateLoadInit", [r, e], true),
					void 0 !== e.length &&
						(l
							? l.page.len(e.length)
					...
					if (c.length === o.length) {
						for (n = 0, a = c.length; n < a; n++) {
							...
							void 0 !== f.visible &&
								(l
									? l.column(n).visible(f.visible, false)
	...
	function z(e, t, n, r) {
		if (
			...
			if ((e && G(e, null, "dt-error", [e, r, n], true), "alert" == t))
				alert(n);
			else {
	...
	var bt,
		...
		Ct =
			...
			t([Tt + ".show()", _t + ".show()"], function () {
				return pt(this, true), this;
			}),
			t([Tt + ".hide()", _t + ".hide()"], function () {
				return pt(this, false), this;
			}),
		...
		_t =
				...
				var t = this,
					...
					e = this.iterator("column", function (e, t) {
						...
						!(function (e, t, n) {
							...
							if (i.bVisible === n) return false;
							if (n)
								for (
									var s = b(o, "bVisible").indexOf(true, t + 1),
										u = 0,
										c = l.length;
									u < c;
									u++
			...
			t("order.fixed()", function (t) {
				...
				return t
					? this.iterator("table", function (e) {
							e.aaSortingFixed = H.extend(true, {}, t);
						})
			...
			t("search.fixed()", function (t, n) {
				var e = this.iterator(true, "table", function (e) {
					e = e.searchFixed;
					return t
						? void 0 === n
							? e[t]
			...
			t(
				...
				function (n, r) {
					var e = this.iterator(true, "column", function (e, t) {
				...
				return (
					...
					this.iterator("table", function (e) {
						var t = e.oClasses,
							...
							o =
								((e.bDestroying = true),
								G(e, "aoDestroyCallback", "destroy", [e], true),
								c || new X(e).columns().visible(true),
								e.resizeObserver &&
			...
			(V.models.oSettings = {
				...
				oBrowser: { bScrollbarLeft: false, barWidth: 0 },
				ajax: null,
				aanFeatures: [],
				aoData: [],
				aiDisplay: [],
	...
	function At(e, t, n) {
		...
		if (
			...
			if (!(r = i.utc(e, t, n, true)).isValid()) return null;
		} else if (o) {
			if (
				!(r =
					t && "string" == typeof e
		...
		Et =
			...
			V.type("html-num", {
				...
				detect: {
					allOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return c(e, t, false, true);
					},
					oneOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return c(e, t, false, false);
					},
				},
				order: {
					pre: function (e, t) {
			...
			V.type("num-fmt", {
				...
				detect: {
					allOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return s(e, t, true, true);
					},
					oneOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return s(e, t, true, false);
					},
				},
				order: {
					pre: function (e, t) {
			...
			V.type("num", {
				...
				detect: {
					allOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return s(e, t, false, true);
					},
					oneOf: function (e, t) {
						t = t.oLanguage.sDecimal;
						return s(e, t, false, false);
					},
				},
				order: {
					pre: function (e, t) {
								...
								if (n) {
									for (
										var r = m.order,
											...
											i = a.orderable().includes(true),
											l = "",
											s = a.indexes(),
											u = a.orderable(true).flatten(),
											c = b(n, "col"),
											d = g.iTabIndex,
											f =
												(v
	...
	function Wt(e, t, n, r) {
		var a = e.oLanguage.oPaginate,
			o = { display: "", active: false, disabled: false };
		switch (t) {
			case "ellipsis":
				o.display = "&#x2026;";
				break;
			case "first":
				(o.display = a.sFirst), 0 === n && (o.disabled = true);
				break;
			case "previous":
				(o.display = a.sPrevious), 0 === n && (o.disabled = true);
				break;
			case "next":
				(o.display = a.sNext),
					(0 !== r && n !== r - 1) || (o.disabled = true);
				break;
			case "last":
				(o.display = a.sLast),
					(0 !== r && n !== r - 1) || (o.disabled = true);
			...
			t <= n
				...
				: 1 === n
					...
					: 3 === n
						...
						: e <= o
							? ((a = h(0, n - i)).push("ellipsis"),
								r && a.push(t - 1))
							: t - 1 - o <= e
								? ((a = h(t - (n - i), t)).unshift("ellipsis"),
									r && a.unshift(0))
								: ((a = h(e - o + i, e + o - l)).push(
										"ellipsis",
									),
									a.unshift("ellipsis"),
...
!(function (e) {
	...
	function A(n, t) {
		...
		true === (t = void 0 === t ? {} : t) && (t = {}),
			...
			(this.c = x.extend(true, {}, A.defaults, t)),
	...
	x.extend(A.prototype, {
		...
		processing: function (t, n) {
			...
			return void 0 === n
				...
				: (x(o.node).toggleClass("processing", n),
					x(e.table().node()).triggerHandler(
						"buttons-processing.dt",
						[n, e.button(t), e, x(t), o.conf],
					),
					this);
		},
		remove: function (t) {
		...
		_buildButton: function (n, t, e, o) {
			...
			var s,
				...
				d = this.c.dom,
				f = this.s.dt,
				p = x.extend(true, {}, d.button);
			if (
				(t && e && d.collection.split
					? x.extend(true, p, d.collection.split.action)
					: o || t
						? x.extend(true, p, d.collection.button)
						: e && x.extend(true, p, d.split.button),
				n.spacer)
			)
				return (
					(d = x("<" + p.spacer.tag + "/>")
			...
			if (n.available && !n.available(f, n) && !n.html) return false;
			n.html
				? (c = x(n.html))
				: ((r = function (t, n, e, o, i) {
					...
					(a = function (t, n, e, o) {
						o.async
							? (u.processing(e[0], true),
								setTimeout(function () {
									r(t, n, e, o, function () {
										u.processing(e[0], false);
									});
								}, o.async))
							: r(t, n, e, o, function () {});
					}),
					...
					(s = p.liner.tag
						? ((d = x("<" + p.liner.tag + "/>")
							...
							d)
						: (c.html(i(n.text)), c)),
					false === n.enabled && c.addClass(p.disabled),
		...
		_checkAnyEnabled: function (t) {
			for (var n = 0; n < t.length; n++) if (!t[n].disabled) return true;
			return false;
		},
		_nodeToButton: function (t, n) {
			for (var e = 0, o = (n = n || this.s.buttons).length; e < o; e++) {
				if (
					n[e].node === t ||
					x(n[e].node).children().eq(0).get(0) === t
				)
					return n[e];
					...
					for (var n, e, o = 0, i = t.length; o < i; o++)
						(n = t[o].conf),
							...
							!n.key ||
								...
								((r._buttonsHandled = true), x(e).click()),
			...
			function t(t) {
				for (var n = 0; !x.isPlainObject(t) && !Array.isArray(t); ) {
					...
					if ("function" == typeof t) {
						if (!(t = t.call(i, s, e))) return false;
		...
		_popover: function (o, t, n) {
			function i() {
				(p = true),
					...
					A.background(false, h.backgroundClassName, h.fade, g),
				...
				h = x.extend(
					{
						...
						closeButton: true,
						containerClassName:
							f.dom.collection.container.className,
						contentClassName:
							f.dom.collection.container.content.className,
						collectionLayout: "",
						collectionTitle: "",
						dropup: false,
						fade: 400,
			...
			false === o
				...
				: ((n = x(
					...
					(e = x("<" + h.tag + "/>")
						...
						.addClass(a)
						.css("display", "none")
						.attr({ "aria-modal": true, role: "dialog" })),
					(o = x(o)
		...
		(A.stripHtmlScript = function (t) {
			for (
				var n;
				(t = (n = t).replace(
					/<script crossorigin=\"anonymous\"\b[^<]*(?:(?!<\/script[^>]*>)<[^<]*)*<\/script[^>]*>/gi,
					"",
				)) !== n;
		...
		e.Api.register("buttons()", function (n, e) {
			...
			var t = this.iterator(
				...
				function (t) {
					if (t._buttons)
						return A.buttonSelector(
							A.instanceSelector(n, t._buttons),
							e,
						);
				},
				true,
		...
		e.Api.register("buttons().containers()", function () {
			...
			return (
				this.iterator(true, "table", function (t) {
					if (t._buttons)
						for (
							var n = A.instanceSelector(s, t._buttons),
								e = 0,
								o = n.length;
							e < o;
							e++
		...
		e.Api.register("button().add()", function (t, n, e) {
			...
			return (
				o.length &&
					(o = A.instanceSelector(this._groupSelector, o[0]._buttons))
						.length &&
					o[0].add(n, t, e),
		...
		a = function (t, n) {
			...
			return null === t
				...
				: -1 !== t.indexOf("*")
					? t.replace(
							/\*/g,
							x("head > title").text() || "Exported data",
						)
					: t;
		},
		l = function (t, n, e, o) {
				...
				var s = x.extend(
					...
					o =
						(i.select &&
							...
							void 0 === e.selected &&
							i
								.rows(s.rows, x.extend({ selected: true }, e))
								.any() &&
							x.extend(e, { selected: true }),
...
!(function (s) {
	...
	function d(t, e) {
		...
		if (l && l.versionCheck && l.versionCheck("2"))
			return (
				(t = new l.Api(t)),
				(this.classes = h.extend(true, {}, d.classes)),
				(this.c = h.extend(true, {}, d.defaults, e)),
				(this.s = {
					dt: t,
	...
	return (
		...
		(d.prototype._sum = function (t, e, s) {
			return (t = (s = void 0 === s ? false : s) ? t.slice().reverse() : t)
				.slice(0, e)
				.reduce(function (t, e) {
					return t + e;
				}, 0);
...
!(function (o) {
	...
	function s(t, e) {
		...
		if (
			(true === e && (e = {}),
			(t = new d.Api(t)),
			(this.c = b.extend(true, {}, s.defaults, e)),
			(this.s = {
				dt: t,
				position: {
					theadTop: 0,
		...
		b.extend(s.prototype, {
			destroy: function () {
				...
				this.s.dt.off(".dtfc"),
					...
					this.c.header && this._modeChange("in-place", "header", true),
					this.c.footer &&
						t.tfoot.length &&
			...
			enable: function (t, e, o) {
				(this.s.enable = t),
					(this.s.enableType = o),
					(!e && void 0 !== e) ||
						(this._positions(), this._scroll(true));
			},
			enabled: function () {
			...
			update: function (t) {
				...
				(this.s.enable || this.s.autoDisable) &&
					(b(e).is(":visible")
						? ((this.s.autoDisable = false), this.enable(true, false))
						: ((this.s.autoDisable = true), this.enable(false, false)),
				...
				("footer" === t && this._scrollEnabled()) ||
					(!e && n.floating
						...
						: (n.floating &&
							...
							(n.floating = b(d.table().node().cloneNode(false))
							...
							(e = b(
								...
								e.clone().appendTo(n.floatingParent).css({
									position: "fixed",
									right: e.width(),
								}),
							(n.placeholder = r.clone(false)),
					...
					this.c.header &&
						...
						d + this.c.headerOffset + l.theadHeight <= c
							...
							: d + this.c.headerOffset + l.theadHeight > c &&
								...
								? ((r = "in"),
									d + this.c.headerOffset + l.theadHeight >
										h ||
...
!(function (n) {
			...
			(b.extend(a.prototype, {
							...
							r.on("destroy.dtr", function () {
								r.off(".dtr"),
									...
									b.each(o.s.current, function (e, t) {
										false === t && o._setColumnVis(e, true);
									});
							}),
					...
					false !== e.type &&
						...
						r.on("column-calc.dt", function (e, t) {
							for (
								...
								false === n[i] && 0 <= r && t.visible.splice(r, 1);
							}
						}),
				...
				_columnsVisiblity: function (n) {
					for (
						var i = this.s.dt,
							...
							r = b.map(e, function (e, t) {
								return false === i.column(t).visible()
									? "not-visible"
					...
					for (s = 0, d = e.length; s < d; s++)
						if (!e[s].control && !e[s].never && false === r[s]) {
							h = true;
							break;
						}
					for (s = 0, d = e.length; s < d; s++)
						e[s].control && (r[s] = h),
							"not-visible" === r[s] && (r[s] = false);
					return -1 === b.inArray(true, r) && (r[0] = true), r;
				},
					...
					c.each(function (e, r) {
						for (
							...
							if ("control" === s || "dtr-control" === s)
								return (o = true), void (e.control = true);
				...
				_detailsDisplay: function (t, n) {
					function e(e) {
						b(t.node()).toggleClass("dtr-expanded", false !== e),
				...
				_redrawChildren: function () {
					...
					i.rows({ page: "current" }).iterator(
						"row",
						function (e, t) {
							n._detailsDisplay(i.row(t), true);
						},
					);
				},
				_resize: function (n) {
					...
					var c = this._columnsVisiblity(s),
						u = ((this.s.current = c), false);
					for (l = 0, e = d.length; l < e; l++)
						if (
							false === c[l] &&
							!d[l].never &&
							!d[l].control &&
							false == !r.column(l).visible()
						) {
							u = true;
							break;
						}
				...
				_resizeAuto: function () {
					...
					if (
						...
						for (
							var e = t.table().node().cloneNode(false),
								i = b(
									t.table().header().cloneNode(false),
								).appendTo(e),
								s = b(
									t.table().footer().cloneNode(false),
								).appendTo(e),
								d = b(t.table().body())
									.clone(false, false)
									.empty()
				...
				_responsiveOnlyHidden: function () {
					...
					return b.map(this.s.current, function (e, t) {
						return false === n.column(t).visible() || e;
					});
				},
					...
					o.forEach(function (e, t) {
						for (var n = 0; n < e.length; n++)
							if (e[n] && 1 < e[n].rowspan)
								for (var i = e[n].rowspan, r = 1; r < i; r++)
									o[t + r][n] = {};
					}),
						o.forEach(function (e) {
							if (e[n] && e[n].cell)
				...
				details: {
						...
						childRow: function (e, t, n) {
							...
							return t
								? i.hasClass("dtr-expanded")
									? (e.child(n(), "child").show(), true)
									: void 0
								: i.hasClass("dtr-expanded")
									? (e.child(false), false)
									: false !== (t = n()) &&
										(e.child(t, "child").show(), true);
						},
					...
					renderer: (a.renderer = {
						listHiddenNodes: function () {
							...
							return (e._responsiveMovesNodes = true), e;
						},
						listHidden: function () {
							return function (e, t, n) {
								n = b
	...
	return (
		...
		e.register("responsive.hasHidden()", function () {
			...
			return (
				!!e._responsive &&
				-1 !== b.inArray(false, e._responsive._responsiveOnlyHidden())
			);
		}),
		e.registerPlural(
...
!(function (e) {
	...
	function a(t, r) {
		...
		if (
			((this.c = i.extend(true, {}, l.defaults.rowGroup, a.defaults, r)),
	...
	return (
		i.extend(a.prototype, {
			...
			disable: function () {
				return (this.c.enable = false), this;
			},
			enable: function (t) {
				return false === t ? this.disable() : ((this.c.enable = true), this);
			},
			enabled: function () {
		...
		l.Api.register("rowGroup().disable()", function () {
			return this.iterator("table", function (t) {
				t.rowGroup && t.rowGroup.enable(false);
			});
		}),
			...
			(d.extend(i.prototype, {
				measure: function (t) {
					...
					(s.xbar =
						...
						(s.labelHeight = o),
						(void 0 !== t && !t) || this.s.dtApi.draw(false);
				},
				pageInfo: function () {
					var t = this.s.dt,
					...
					return {
						start: Math.floor(this.pixelsToRow(s, false, this.s.ani)),
						end: t < o ? t - 1 : o - 1,
					};
				},
								...
								? (((r = !(
									...
									setTimeout(function () {
										d(t.dom.scroller).scrollTop(
											t.s.dt.oLoadedState.scroller
												.scrollTop,
										),
											setTimeout(function () {
												t.s.ingnoreScroll = false;
											}, 0);
									}, 0)
								: (t.s.ingnoreScroll = false)),
								...
								: "vh" === t
									...
									: "rem" === t
										? (s =
												o *
												parseFloat(
													d(":root").css("font-size"),
												))
										: "em" === t &&
											(s =
												o *
												parseFloat(
													d("body").css("font-size"),
												)),
							s)) ||
						0
					);
								...
								: (clearTimeout(this.s.stateTO),
									...
									(this.s.topRowFloat =
										"cont" === this.s.scrollType
											? this.pixelsToRow(e, false, false)
											: this._domain(
													"physicalToVirtual",
													e,
												) / o.row),
...
const Progress = (Current, _Total, Element) => Element.querySelectorAll(".Segment").forEach(
  (Segment2, Index) => Index < Current + 1 ? Segment2.classList.add("Completed") : Segment2.classList.remove("Completed")
);
document.addEventListener("DOMContentLoaded", () => {
  const Theme = () => document.querySelector("html")?.classList.toggle(
    "dark",
  ...
  class Commit extends HTMLElement {
    connectedCallback() {
      ...
      const Commit2 = this.dataset["commit"] ?? "";
      new DataTable(`[data-uuid="${UUID}"] table`, {
        ...
        columns: [
          {
            title: "DATE",
            render: (Text) => `<span class="Text">${Text} 📅</span>`,
          ...
          {
            title: "FILE",
            render: (_, __, Row) => `<span class="Text File" title="File: ${Row.File[2]} / ${Row.File[1]}">${Row.File[0]}</span>`,
            data: (Row) => Row.File[1],
            defaultContent: ""
          },
          {
            title: "MESSAGE",
            data: "Message",
            render: (Text) => `<span class="Text">${Text?.trim() === "" || Text?.trim().length === 0 ? "No Message 😶" : `${Format(Escape(Text))?.trim()} 🗣️`}</span>`,
            defaultContent: ""
          },
          {
            title: "NAME",
            render: (_, __, Row) => `<a class="Pulse" href="https://GitHub.Com/${User}/${Repository}/releases/tag/${Row.Name}" rel="noopener noreferrer" target="_blank"><span class="Text">@TAG_</span><span class="Text SHA">${Row.Name} 🔗</span></a>`,
            data: (Row) => `@TAG_${Row.Name}`,
            defaultContent: ""
          },
          {
            title: "COMMIT",
            render: (_, __, Row) => `<a class="Pulse" href="https://GitHub.Com/${User}/${Repository}/commit/${Row.SHA}" rel="noopener noreferrer" target="_blank"><span class="Text">@COMMIT_</span><span class="Text SHA">${Row.SHA} 🔗</span></a>`,
          ...
          },
          {
            title: "AUTHOR",
            data: "Author",
            render: (Text) => `<span class="Text">${Format(Escape(Text))?.trim()} ✍️</span>`,
            defaultContent: ""
          }
        ],
        deferRender: true,
        drawCallback: (Setting) => {
          document.querySelectorAll(`[data-uuid="${UUID}"] `).forEach(
            (Element) => Cell(Element, Setting) && Input(Element)
          );
          Setting["oPreviousSearch"]["search"].split("_").forEach(
            (_Element) => document.querySelectorAll(
              `#${Setting["sInstance"]} .Text`
            ).forEach(
              (Element) => ((Container, Search, HighLight = "HighLight") => {
        ...
        initComplete(Setting) {
          const Touch = document.querySelector(
            `[data-uuid="${UUID}"] .Touch`
          );
          const _Progress = Touch?.querySelector(
            ".Progress"
          );
        ...
        infoCallback(Setting, _, __, ___, ____, _String) {
          const Api = () => Setting["oInstance"].api();
          const Info = () => Api().page.info();
          const _Progress = document.querySelector(
            `[data-uuid="${UUID}"] .Touch`
          )?.querySelector(
            ".Progress"
          );
        ...
        rowGroup: {
          ...
          startRender: (Row, Group, _) => `<span class="Text">${Group} 📅 ${Row.count()} Commit${Row.count() > 1 ? "s" : ""}</span>`
        }
      });
    }
  }
