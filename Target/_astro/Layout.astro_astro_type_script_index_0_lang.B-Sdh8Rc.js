...
const RGB = (...[HEX]) => {
  HEX = HEX.replace(/^#/, "");
  if (HEX.length === 3) {
    HEX = HEX.split("").map((String) => String + String).join("");
  }
...
const Segment = (Total, Element) => {
  Element.innerHTML = "";
  Element.querySelectorAll(".Segment").forEach((Element2) => Element2.remove());
...
function requireJquery () {
	...
	(function (module) {
		...
		var
			...
			rnoInnerhtml = /<script crossorigin=\"anonymous\"|<style|<link/i,
			...
			if ( s.crossDomain || s.scriptAttrs ) {
				...
				return {
					send: function( _, complete ) {
						script = jQuery( "<script crossorigin=\"anonymous\">" )
							.attr( s.scriptAttrs || {} )
...
var _stripHtml = function (input) {
	...
	do {
		previous = input;
		input = input.replace(/<script crossorigin="anonymous"/i, '');
      ...
      new DataTable(`[data-uuid="${UUID}"] table`, {
        ...
        columns: [
          ...
          },
          {
            title: "MESSAGE",
            data: "Message",
            render: (Text) => `<span class="Text">${Text.trim() === "" || Text.trim().length === 0 ? "No Message 😶" : `${Text.trim()} 🗣️`}</span>`
          },
          {
            title: "COMMIT",
          ...
          },
          {
            title: "AUTHOR",
            data: "Author",
            render: (Text) => `<span class="Text">${Text} ✍️</span>`
          }
        ],
        deferRender: true,
        drawCallback: (Setting) => Setting["oPreviousSearch"]["search"].split("_").forEach(
...
//# sourceMappingURL=Layout.astro_astro_type_script_index_0_lang.B-Sdh8Rc.js.map
